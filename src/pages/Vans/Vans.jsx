  ...
  return (
    <div className="py-8 flex flex-col gap-8 px-8 md:px-16">
      ...
      <Await resolve={loaderData}>
        {(loadedData) => {
          const vans = loadedData.vans;

          const vanElements = filteredVans.map((van) => (
            <div key={van.id} className="w-full flex flex-col gap-4 relative">
              <Link
                to={van.id}
                state={{ search: searchParams.toString(), filter: typeFilter }}
                className="w-full h-full"
              >
                <img
                  alt={van.van}
                  src={van.imageUrl}
                  className="w-full aspect-square object-cover object-center clip-van border-1 border-orange-200"
                />
              </Link>
              <div className="w-full flex items-center justify-between">
                ...
                <p className="font-bold">
                  ${van.price}
                  <small className="font-normal">/day</small>
                </p>
              </div>
              <i
                className={`absolute ${typeColor(
                  van.type
                )} p-1 px-3 clip-van van-type ${van.type} selected text-white`}
              >
                {van.type}
              </i>
            </div>
            ...
            <>
              <div className="flex gap-2 items-center">
                <button
                  ...
                  className={`p-1 px-4 clip-van text-white text-sm ${
                    typeFilter === null ? `bg-neutral-900` : `bg-neutral-600`
                  }`}
                >
                  all
                </button>
